...
export const updateScheduleChangeRequestStatus = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Đã ${action === "approve" ? "phê duyệt" : "từ chối"} yêu cầu thành công`,
...
export const addMakeupSchedule = async (req, res) => {
  try {
    ...
    if (!date || !startTime || !endTime || !location) {
      return res.status(400).json({
        success: false,
        message: "Vui lòng cung cấp đầy đủ thông tin: date, startTime, endTime, location"
      });
    }
    ...
    if (request.class) {
      ...
      if (makeupDate < classStartDate || makeupDate > classEndDate) {
        return res.status(400).json({
          success: false,
          message: `Ngày dạy bù phải nằm trong thời gian lớp học (${classStartDate.toLocaleDateString('vi-VN')} - ${classEndDate.toLocaleDateString('vi-VN')})`
        });
      }
    }
