...
class NotificationService {
  ...
  static async notifyUserPaymentApproved(payment, admin) {
    ...
    return await this.createNotification({
      ...
      message,
      type: "payment",
      recipient: payment.user || payment.userId,
      sender: admin._id,
      relatedId: payment._id,
    });
  }

  // Thông báo học viên thanh toán bị từ chối
  static async notifyUserPaymentRejected(payment, admin, rejectionReason) {
    const title = "Thanh toán bị từ chối";
    const message = rejectionReason
      ? `Thanh toán của bạn đã bị từ chối. Lý do: ${rejectionReason}`
      : "Thanh toán của bạn đã bị từ chối bởi Admin. Vui lòng liên hệ để biết thêm chi tiết.";
